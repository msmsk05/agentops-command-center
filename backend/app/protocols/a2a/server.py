...
# Legacy path kept for backward compatibility with pre-1.0 A2A clients.
...
def mount_a2a(app: FastAPI, public_url: str) -> None:
    ...
    # create_agent_card_routes registers the standards-compliant AGENT_CARD_WELL_KNOWN_PATH
    # ("/.well-known/agent-card.json" for A2A 1.0); append the legacy path alongside it.
    card_routes = create_agent_card_routes(card, card_url=AGENT_CARD_WELL_KNOWN_PATH)
    card_routes.append(Route(LEGACY_AGENT_CARD_PATH, endpoint=compatibility_card, methods=['GET']))
